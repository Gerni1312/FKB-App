...
const STORAGE_KEY = "sigma-finance-app-cra-v1";
...
function App() {
    ...
    }, [transactions, budgets, recurring, goals, savingsAccount, currency, weeklyMode, monthOffset]);
    ...
    function settleBorrowedSavings() {
        ...
        setSavingsAccount((prev) => ({ ...prev, balance: Number(prev.balance || 0) + Number(prev.expectedInterest || 0), borrowedOut: 0, expectedInterest: 0 }));
    }

    function exportData() {
        downloadJson(`sigma-finance-${selectedMonthKey}.json`, { exportedAt: new Date().toISOString(), version: 1, transactions, budgets, recurring, goals, savingsAccount, settings: { currency, weeklyMode, monthOffset } });
    }
            ...
            <div style={s.container}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
                        <div>
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                                <div style={{ fontSize: 12, letterSpacing: 3, textTransform: "uppercase", opacity: 0.65 }}>FKB</div>
                            ...
                            <div style={{ marginTop: 10, maxWidth: 760, color: "rgba(255,255,255,0.78)", fontSize: 15 }}>
                                Gleiche starke Finanz-App, aber ohne den komplizierten UI-Unterbau. Läuft direkt in deinem normalen React-Projekt.
                            </div>
                        </div>
                ...
                {tab === "dashboard" && (
                    <>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 16 }}>
                            ...
                            <StatCard title="Sparen" value={money(totals.saving, currency)} subValue={`${percentages.savingPct.toFixed(0)}% Sparquote`} hint="Im Monat zur Seite gelegt" icon={PiggyBank} gradient="linear-gradient(135deg,#0369a1,#38bdf8)" />
                            <StatCard title="Sparkonto" value={money(savingsAccount.balance, currency)} hint="Aktueller Stand auf dem Sparkonto" icon={Activity} gradient="linear-gradient(135deg,#b45309,#f59e0b)" />
                        </div>
                ...
                {tab === "transactions" && (
                    <div style={{ ...s.card, padding: 18 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr repeat(2, minmax(180px, 220px))", gap: 12 }}>
                            ...
                            <button style={s.button} onClick={() => setShowAddTransaction(true)}>Schnell hinzufügen</button>
                        </div>
                ...
                {tab === "goals" && (
                    <div style={{ display: "grid", gap: 16 }}>
                        ...
                        <div style={{ ...s.card, padding: 18 }}>
                            <SectionTitle title="Sparkonto & Ausleihen" description="Wenn du Geld vom Sparkonto nimmst und am Monatsanfang zurücklegst" />
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px,1fr))", gap: 16 }}>
                                <div style={s.softCard}>
                                    ...
                                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginTop: 14 }}>
                                        <div style={{ ...s.softCard, background: "white" }}><div style={{ fontSize: 13, color: "#71717a" }}>Frei drauf</div><div style={{ fontWeight: 800, marginTop: 5 }}>{money(savingsSummary.availableOnSavings, currency)}</div></div>
                                        ...
                                        <div style={{ ...s.softCard, background: "white" }}><div style={{ fontSize: 13, color: "#71717a" }}>Prognose</div><div style={{ fontWeight: 800, marginTop: 5 }}>{money(savingsSummary.projectedNextMonth, currency)}</div></div>
                                    </div>
                                </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px,1fr))", gap: 16 }}>
                            ...
                            <div style={s.softCard}>
                                <div style={{ display: "flex", gap: 10, alignItems: "start" }}>
                                    ...
                                    <div>
                                        <div style={{ fontWeight: 800 }}>Sparkonto integriert</div>
                                        <div style={{ fontSize: 14, color: "#71717a", marginTop: 4 }}>Stand, geplante Sparrate, geliehene Beträge und Zins sind direkt drin.</div>
                                    </div>
                                </div>
                            </div>
                        </div>
